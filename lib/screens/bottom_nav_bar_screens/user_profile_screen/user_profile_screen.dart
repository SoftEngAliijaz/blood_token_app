...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  Future<void> showModalBottomSheetSuggestions(BuildContext context) async {
  ...
  Future<void> _updateProfile(String userId) async {
    ...
    try {
      ...
      if (_pickedImage != null) {
        final downloadURL = await uploadImageAndGetDownloadURL(_pickedImage);
        if (downloadURL != null) {
                          ...
                          Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                profileCard('Age', '${user['age'] ?? ''}'),
                                profileCard('Blood Group',
                                    '${user['bloodGroup'] ?? ''}'),
                                profileCard(
                                    'Phone', '+92${user['phoneNumber'] ?? ''}'),
                              ],
                            ),
                          ),
                        ],
